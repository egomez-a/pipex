...
/*   Updated: 2021/11/29 13:11:04 by egomez-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_program(t_pipex pipex, char **cmd, int *check)
{
	if (access(cmd[0], X_OK) == 0 && check[0] == 0)
	{
		...
		printf("La cadena cmd1 es ejecutable con %s\n", cmd[0]);
		check[0] = 1;
	}
	if (access(cmd[1], X_OK) == 0 && check[1] == 0)
	{
		...
		printf("La cadena cmd2 es ejecutable con %s\n", cmd[1]);
		check[1] = 1;
	}
}
	...
	while (pipex.path[i])
	{
		...
		{	if (pipex.cmd1[0][0] == '/')
			...
			else
			{
				...
				printf("Para i = %d la cadena cmd[0] es %s\n", i, cmd[0]);
			}
		}
		if (check[1] == 0)
		{
			...
			else
			{
				...
				printf("Para i = %d la cadena cmd[1] es %s\n", i, cmd[1]);
			}
		}
